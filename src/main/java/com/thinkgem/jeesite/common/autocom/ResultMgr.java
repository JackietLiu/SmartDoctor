...
public class ResultMgr {
	...
	private String brToJson() {
		...
		jsonConfig.setExcludes(new String[] { "opt","inqu","option","sym","question","office","createBy","updateBy","currentUser","page","sqlMap","createDate","updateDate" });
	...
	private String lrToJson() {
		JsonConfig jsonConfig = new JsonConfig(); // 建立配置文件
		jsonConfig.setIgnoreDefaultExcludes(false); // 设置默认忽略
		jsonConfig.setExcludes(new String[] { "opt","inqu","option","sym","question","office","createBy","updateBy","currentUser","page","sqlMap","createDate","updateDate" }); // 此处是亮点，只要将所需忽略字段加到数组中即可，在上述案例中，所要忽略的是“libs”，那么将其添到数组中即可，在实际测试中，我发现在所返回数组中，存在大量无用属性，如“multipartRequestHandler”，“servletWrapper”，那么也可以将这两个加到忽略数组中.
